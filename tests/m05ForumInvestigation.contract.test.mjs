...
test('M05 live console preserves the Tick 0-10 Century Gym replay and accepts bounded live scenarios', () => {
  ...
  assert.match(page, /启动模拟/)
  const navigation = read('src/campus-pulse/app/navigation.js')
  const investigationPage = read('src/campus-pulse/forum/ForumInvestigationPage.vue')
  const landingPage = read('src/campus-pulse/live/LiveEvolutionLandingPage.vue')
  assert.match(navigation, /id: 'forum'[\s\S]{0,180}name: 'campus-pulse-live-evolution'/)
  assert.match(router, /path: '\/campus-pulse\/live-evolution'[\s\S]{0,180}name: 'campus-pulse-live-evolution'/)
  assert.match(landingPage, /name:'campus-pulse-century-gym-live'/)
  assert.match(landingPage, /name:'campus-pulse-workbench'/)
  assert.match(landingPage, /scenario:'governance_legitimacy_dispute'/)
  assert.match(landingPage, /scenario:'lecture_external_incident_shock'/)
  assert.match(landingPage, /These are not completed user-project runs/)
  assert.doesNotMatch(landingPage, /project_id/)
  assert.doesNotMatch(page, /Century Gym audited demo|openCenturyGymDemo/)
  assert.doesNotMatch(investigationPage, /Century Gym audited demo|forum-century-gym-demo-title/)
