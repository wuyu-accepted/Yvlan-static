...
const props = withDefaults(defineProps<{
  ...
  auditedReplaySummary?: AuditedReplaySummary | null
}>(), {
  projectId: '',
  variant: 'overview',
  runtimeTick: null,
...
const copy = computed(() => isEnglish.value ? {
  ...
  activation: 'Activation scheduler', preview: 'Pre-run schedule preview', tick: 'Tick', mode: 'Run mode', active: 'scheduled activation', committed: 'committed agent output', queueNote: 'At runtime, direct replies, corrections, service receipts, risk/help signals and private inboxes override this baseline projection before stratified PPS fills the remaining slots.',
  ...
  fixed: '固定人口版本', agents: 'LLM Agent', particles: '状态粒子', roles: '宏观角色', micro: '微角色池', represented: '实际覆盖', edges: '合成关系',
  ...
  activation: '节点激活调度器', preview: '运行前调度预演', tick: '时间步', mode: '运行模式', active: '调度预演激活', committed: '已提交 Agent 输出', queueNote: '真实运行时，直接回复、纠错、服务回执、风险/求助和私聊收件箱会先覆盖本基线预演，剩余名额再由分层轮换 PPS 补齐。',
  ...
  retry: '重试', unavailable: 'Agent 世界版本加载失败。', hash: '世界版本', census: '1,000 人全量', activated: '已激活', background: '后台状态', agentsInRole:'这个角色中的活 Agent', openDossier:'打开完整档案', dossierNote:'人物资料 · Prompt · 记忆 · 历史', inspectable:'个可检查的活 Agent', projected:'调度预演激活', loadingDossier:'正在读取档案…', particleHalo:'光晕 = 10 个状态粒子的不确定性',
})
...
const projectedRuntimeFrame = computed<ForumWorldRuntimeFrame | null>(() => {
  ...
  for (const event of props.runtimeEvents) {
    ...
    for (const [index,raw] of edgeRows.entries()) {
      ...
      liveEdges.push({id:`event-edge:${event.sequence}:${index}`,sourceId:source,targetId:target,channel:rawChannel as 'public'|'reply'|'private_direct'|'private_group'|'governance',count:Math.max(1,Number(row.count || row.message_count || 1)),label:String(row.label || event.message || event.kind),evidence:text?[{id:`event:${event.sequence}`,kicker:event.kind,text,provenance:String(payload.provenance || 'runtime event')}]:[]})
    }
  }
  const nodes=agentNodes.value.map(({agent,x,y})=>({
    ...
    status:runtimeEvidence.has(agent.display_id)?copy.value.committed:activeAgentIds.value.has(agent.display_id)?copy.value.active:(isEnglish.value?'Background state':'后台状态'),
  ...
  const completed=props.runtimeTurnsCompleted,reserved=props.runtimeTurnsReserved ?? 0
  ...
  const unavailable=isEnglish.value?'Unavailable':'不可用'
  const replay=props.auditedReplaySummary
  const metrics:ForumWorldRuntimeFrame['metrics']=replay ? [
    {id:'scheduled',label:isEnglish.value?'Scheduled activation':'调度预演激活',value:activeAgentIds.value.size,note:`/ ${world.value.population.agent_count}`},
    {id:'committed',label:isEnglish.value?'Committed agent output':'已提交 Agent 输出',value:unavailable,help:isEnglish.value?'Record-level output not published\nReplay aggregate only':'未发布记录级输出\n仅提供回放聚合'},
    {id:'turns',label:isEnglish.value?'Completed turns':'已完成轮次',value:completed ?? unavailable,help:completed===null||completed===undefined?(isEnglish.value?'Turn count not published':'未发布完成轮次数'):undefined},
    {id:'public',label:isEnglish.value?'Committed public messages':'已提交公开消息',value:replay.committedPublicMessages ?? unavailable},
    {id:'activations',label:isEnglish.value?'Published activations':'已发布激活',value:replay.residentActivations===null&&replay.governanceActivations===null?unavailable:`${replay.residentActivations ?? '—'} + ${replay.governanceActivations ?? '—'}`,help:isEnglish.value?'Resident + governance\nSelected replay Tick':'居民 + 治理\n当前回放 Tick'},
    {id:'private-messages',label:isEnglish.value?'Private messages':'私域消息',value:replay.privateMessages ?? unavailable,help:isEnglish.value?'Aggregate count\nSelected replay Tick':'聚合计数\n当前回放 Tick'},
    {id:'private-role-edges',label:isEnglish.value?'Private role edges':'私域角色边',value:replay.privateRoleEdges ?? unavailable,help:isEnglish.value?'Role-level aggregate\nNo agent identities inferred':'角色级聚合\n不推断 Agent 身份'},
    {id:'replay-timeline',label:isEnglish.value?'Replay timeline records':'回放时间线记录',value:replay.timelineRecords ?? unavailable},
  ] : [
    {id:'scheduled',label:isEnglish.value?'Scheduled activation':'调度预演激活',value:activeAgentIds.value.size,note:`/ ${world.value.population.agent_count}`},
    {id:'committed',label:isEnglish.value?'Committed agent output':'已提交 Agent 输出',value:runtimeEvidence.size},
    {id:'turns',label:isEnglish.value?'Completed turns':'已完成轮次',value:completed ?? unavailable,note:reserved?`/ ${reserved}`:undefined},
    ...
    {id:'queue',label:isEnglish.value?'API runtime events':'API 运行事件',value:props.runtimeEvents.length},
  ]
  return {
    ...
    boundaryNote:isEnglish.value?'The view updates from runtime progress and committed event payloads. Missing messages or private links remain empty rather than being invented.':'视图只依据运行进度与已提交事件更新；尚未发布的帖子、私聊或群聊保持为空，不由前端补造。',
  }
})
...
<template>
  <template v-if="variant === 'runtime' && projectedRuntimeFrame">
    <ForumWorldRuntimeStage :frame="projectedRuntimeFrame" @inspect-node="inspectRuntimeNode" />
    <AgentDossierDrawer :open="dossierOpen" :agent="selectedAgent" :scenario-label="scenarioNames[selectedScenarioId] || selectedScenarioId" :branch-label="runtimeBranchLabel || copy.preview" :tick="effectiveTick" :status-label="runtimeEvents.some(event => String(event.payload?.display_id || event.payload?.agent_display_id || '') === selectedAgent?.display_id) ? copy.committed : activeAgentIds.has(selectedAgent?.display_id || '') ? copy.active : copy.background" :project-id="props.projectId" editable @close="dossierOpen=false" />
  </template>
    ...
    <template v-else>
      ...
      <div class="world-grid">
        <section class="activation-card">
          <header><div><p>{{ copy.preview }}</p><h3>{{ copy.activation }}</h3><code>{{ world.activation.algorithm_id }}</code></div><strong>{{ currentBudget.toLocaleString() }} <small>/ {{ world.population.agent_count }}</small></strong></header>
      ...
      <footer class="world-proof"><span>{{ copy.hash }} <code>{{ shortHash(world.world_sha256) }}</code></span><span>Profile {{ world.population.profile_bundle_count.toLocaleString() }} · {{ world.schema_version }}</span></footer>
...
.agent-world{gap:var(--cp-space-3);padding:var(--cp-space-4);border-color:var(--cp-border-graphite);background:var(--cp-canvas-obsidian);box-shadow:none;color:var(--cp-text-warm);color-scheme:dark}.world-head{align-items:center;gap:var(--cp-space-5)}.world-head p,.activation-card header p,.channel-card header p,.risk-card>div>p{color:var(--cp-accent-crimson);font-size:var(--cp-text-xs);letter-spacing:.08em}.world-head h2{color:var(--cp-text-warm);font-size:clamp(1.5rem,2.2vw,2.15rem);line-height:1.15;letter-spacing:-.025em}.world-head>div>span{max-width:68rem;margin-top:var(--cp-space-2);color:var(--cp-text-warm-muted);font-size:var(--cp-text-sm);line-height:1.6}.configure,.open-dossier{border-color:var(--cp-accent-crimson);background:var(--cp-accent-crimson);color:var(--cp-text-warm)}.configure:hover,.open-dossier:hover{background:var(--cp-accent-crimson-hover)}.world-loading{padding:4rem}.world-loading span{background:var(--cp-accent-crimson)}.world-error{border-color:var(--cp-risk-high);background:color-mix(in srgb,var(--cp-risk-high) 10%,var(--cp-surface-charcoal))}.world-error button{border-color:var(--cp-risk-high);background:var(--cp-surface-charcoal);color:var(--cp-risk-high)}.world-stats{border-color:var(--cp-border-graphite)}.world-stats>div{padding:var(--cp-space-3) var(--cp-space-4);border-color:var(--cp-border-graphite)}.world-stats small,.world-stats span{color:var(--cp-text-warm-muted)}.world-stats strong{color:var(--cp-text-warm);font-size:clamp(1.45rem,2.4vw,2rem)}.network-card,.activation-card,.stake-card,.channel-card,.risk-card{border-color:var(--cp-border-graphite);background:var(--cp-surface-charcoal)}.network-head{padding:var(--cp-space-3) var(--cp-space-4);border-color:var(--cp-border-graphite)}.network-head h3,.activation-card h3,.stake-card h3,.channel-card h3,.risk-card h3{color:var(--cp-text-warm)}.network-head p{color:var(--cp-text-warm-muted)}.relation-filter button{border-color:var(--cp-border-graphite);background:var(--cp-surface-charcoal-raised);color:var(--cp-text-warm-muted)}.relation-filter button.active{border-color:var(--cp-accent-crimson);background:color-mix(in srgb,var(--cp-accent-crimson) 13%,var(--cp-surface-charcoal));color:var(--cp-text-warm)}.dot{background:var(--cp-text-warm-muted)}.dot.close{background:var(--cp-accent-crimson)}.dot.familiar{background:var(--cp-evidence)}.dot.cross{background:color-mix(in srgb,var(--cp-branch-natural) 58%,var(--cp-text-warm-muted))}.network-layout{min-height:29rem}.network-stage{background:linear-gradient(color-mix(in srgb,var(--cp-border-graphite) 35%,transparent) 1px,transparent 1px),linear-gradient(90deg,color-mix(in srgb,var(--cp-border-graphite) 35%,transparent) 1px,transparent 1px),var(--cp-canvas-obsidian);background-size:48px 48px}.edges line,.relationship-edge .relationship-line{stroke:var(--cp-text-warm-muted);opacity:.23}.edges line.cross,.relationship-edge.cross .relationship-line{stroke:color-mix(in srgb,var(--cp-branch-natural) 70%,var(--cp-text-warm-muted));opacity:.45}.node-core{fill:var(--cp-surface-charcoal-raised);stroke:var(--cp-text-warm-muted)}.node text{fill:var(--cp-text-warm)}.node .node-count{fill:var(--cp-text-warm-muted)}.node.active .node-core{fill:var(--cp-accent-crimson);stroke:var(--cp-text-warm)}.node.selected .node-core{stroke:var(--cp-evidence)}.pulse,.micro-pulse{stroke:var(--cp-accent-crimson);filter:none}.network-center circle{fill:var(--cp-surface-charcoal);stroke:var(--cp-border-graphite)}.network-center .scan{stroke:var(--cp-accent-crimson);opacity:.35}.network-center text{fill:var(--cp-text-warm)}.network-center .center-count{fill:var(--cp-evidence)}.network-center .center-sub{fill:var(--cp-text-warm-muted)}.role-inspector{gap:var(--cp-space-3);padding:var(--cp-space-4);border-color:var(--cp-border-graphite);background:var(--cp-surface-charcoal-raised)}.role-inspector>p{color:var(--cp-accent-crimson)}.role-inspector h4,.role-numbers strong{color:var(--cp-text-warm)}.role-numbers span,.role-inspector section small,.role-agents>p{color:var(--cp-text-warm-muted)}.role-inspector em,.role-agents button{border-color:var(--cp-border-graphite);background:var(--cp-surface-charcoal);color:var(--cp-text-warm)}.role-agents button.selected{border-color:var(--cp-accent-crimson);background:color-mix(in srgb,var(--cp-accent-crimson) 13%,var(--cp-surface-charcoal))}.role-agents button span{color:var(--cp-text-warm-muted)}.edge-inspector-note{border-color:var(--cp-accent-crimson);background:color-mix(in srgb,var(--cp-accent-crimson) 9%,var(--cp-surface-charcoal));color:var(--cp-text-warm-muted)}.micro-agent-node .micro-core{fill:var(--cp-text-warm-muted);stroke:var(--cp-surface-charcoal);filter:none}.micro-agent-node.active .micro-core{fill:var(--cp-accent-crimson);stroke:var(--cp-text-warm);filter:none}.micro-agent-node.selected .micro-core,.micro-agent-node:focus-visible .micro-core,.relationship-edge.selected .relationship-line,.relationship-edge:focus-visible .relationship-line{stroke:var(--cp-focus-ring)}.agent-node-caption{border-color:var(--cp-border-graphite);background:var(--cp-surface-charcoal-raised);color:var(--cp-text-warm-muted);box-shadow:none}.agent-node-caption b{color:var(--cp-accent-crimson)}.particle-halo{fill:var(--cp-branch-natural);opacity:.16!important}.micro-agent-node.active .particle-halo{fill:var(--cp-evidence);opacity:.25!important}.activation-card,.stake-card,.channel-card,.risk-card{padding:var(--cp-space-4)}.activation-card header strong,.stake-summary strong{color:var(--cp-accent-crimson)}.activation-card header small,.activation-controls label,.stake-summary span,.stake-bars span,.stake-bars em,.risk-card>div>span{color:var(--cp-text-warm-muted)}.activation-controls input,.activation-controls select,.stake-card header select{border-color:var(--cp-border-graphite);background:var(--cp-surface-charcoal-raised);color:var(--cp-text-warm);accent-color:var(--cp-accent-crimson)}.activation-flow span{color:var(--cp-text-warm-muted)}.activation-flow b{background:var(--cp-canvas-obsidian);color:var(--cp-text-warm)}.activation-flow>i{color:var(--cp-border-graphite)}.queue-note{border-color:var(--cp-accent-crimson);background:var(--cp-surface-charcoal-raised);color:var(--cp-text-warm-muted)}.stake-bars i{background:var(--cp-border-graphite)}.stake-bars b{background:var(--cp-accent-crimson)}.channel-flow article{min-height:7rem;padding:var(--cp-space-3);border-color:var(--cp-border-graphite);background:var(--cp-surface-charcoal-raised)}.channel-flow article>i{background:var(--cp-canvas-obsidian);color:var(--cp-text-warm)}.channel-flow p{color:var(--cp-text-warm-muted)}.channel-flow em{border-color:var(--cp-evidence);color:var(--cp-evidence)}.channel-link i{background:var(--cp-accent-crimson)}.risk-card li{border-color:var(--cp-border-graphite);color:var(--cp-text-warm-muted)}.risk-card li b{color:var(--cp-risk-high)}.risk-lists section{border-color:var(--cp-border-graphite);background:var(--cp-canvas-obsidian)}.risk-lists section small{color:var(--cp-evidence)}.risk-lists section span{color:var(--cp-text-warm-muted)}.world-proof,.world-proof code{color:var(--cp-text-warm-muted)}
