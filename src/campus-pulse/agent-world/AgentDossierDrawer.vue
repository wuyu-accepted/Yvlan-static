...
import type { AgentBehaviorChainStep, AgentDossierEvent, AgentDossierPrivateExcerpt, LiveAgentProfileOverlay, PublicAgentDossier } from './types.ts'
...
const props = withDefaults(defineProps<{
  ...
  promptHash?:string|null; events?:AgentDossierEvent[]; relatedPrivateExcerpts?:AgentDossierPrivateExcerpt[]; behaviorChain?:AgentBehaviorChainStep[]
  projectId?:string; editable?:boolean
}>(), {
  liveProfile:null, scenarioLabel:'', branchLabel:'', tick:null, statusLabel:'', promptHash:null,
  events:() => [], relatedPrivateExcerpts:() => [], behaviorChain:() => [], projectId:'', editable:false,
})
const emit = defineEmits<{ close:[] }>()
type Tab = 'now'|'chain'|'profile'|'prompt'|'memory'|'history'
...
const sortedEvents = computed(() => props.events.filter(item => props.tick === null || item.tick <= props.tick).sort((a,b) => b.tick-a.tick || b.id.localeCompare(a.id)))
const recentMemories = computed(() => sortedEvents.value.filter(item => item.text && (props.tick === null || item.tick < props.tick)).slice(0,2))
...
const tabs = computed<Array<{key:Tab;label:string}>>(() => [
  {key:'now',label:l('此刻','Now')},{key:'chain',label:l('行为链','Action chain')},{key:'profile',label:l('人物资料','Agent profile')},
...
async function saveProfileDraft():Promise<void> {
  ...
  try {
    ...
    workingProfileDraft.value = null
    editingProfile.value = false
    profileSaveMessage.value = l('人物设定已保存，将用于本项目的下一次运行。','Profile saved for this project’s next run.')
  } catch {
    profileSaveMessage.value = l('人物设定保存失败，请重试。','Could not save the profile. Please retry.')
  } finally {
...
async function restoreBaseProfile():Promise<void> {
  ...
  try {
    ...
    workingProfileDraft.value = null
    editingProfile.value = false
    profileSaveMessage.value = l('已恢复该 Agent 的初始人物资料。','The Agent’s initial profile has been restored.')
  } catch {
    profileSaveMessage.value = l('人物资料恢复失败，请重试。','Could not restore the profile. Please retry.')
  } finally {
    profileSaving.value = false
  }
}
...
watch([() => props.open, displayId, () => props.projectId], ([open]) => {
  if (!open) return
  tab.value=props.behaviorChain.length >= 2 ? 'chain' : 'now'
  loadProfileDraft()
}, { immediate:true })
  ...
  <Teleport to="body"><Transition name="dossier"><div v-if="open" class="dossier-layer" role="dialog" aria-modal="true" :aria-label="l('合成 Agent 档案','Synthetic Agent dossier')" @click.self="emit('close')">
    <article class="dossier-card">
      ...
      <div class="dossier-body">
        ...
        <section v-else-if="tab==='chain'" class="section-stack chain-section">
          <header><small>AGENT ACTION CHAIN</small><h3>{{ l('从看到什么，到后来怎么做','From observed context to subsequent action') }}</h3><p>{{ l('按时间串联人物依据、当轮可见信息、实际表达、直接回应与后续行为。','A chronological chain linking the Agent basis, visible context, actual expression, direct responses and subsequent behaviour.') }}</p></header>
          <article class="chain-basis"><b>00 · {{ l('人物与历史','Profile and history') }}</b><p>{{ displayedProfile.roleDescription }} {{ displayedProfile.stableBackground }}</p></article>
          <ol v-if="behaviorChain.length" class="behaviour-chain">
            <li v-for="step in behaviorChain" :key="step.id">
              <time>Tick {{ step.tick }}</time>
              <div class="chain-stage"><b>01 · {{ l('本轮实际看到的信息','Visible information this turn') }}</b><template v-if="step.visibleContext.length"><p v-for="context in step.visibleContext" :key="context.id"><small>{{ context.kind==='thread_root' ? l('讨论串原帖','Thread root') : context.kind==='direct_parent' ? l('直接父消息','Direct parent') : l('引用消息','Quoted message') }} · {{ context.speaker }}</small>{{ context.text }}</p></template><p v-else class="chain-empty">{{ l('本轮公开账本没有父消息或引用上下文。','No parent or quoted context is recorded in the public ledger for this turn.') }}</p></div>
              <div class="chain-stage chain-action"><b>02 · {{ l('产生的操作和文本','Action and text produced') }}</b><p><small>{{ channelLabel(step.action.channel) }} · {{ actionLabel(step.action.action) }}</small>{{ step.action.text }}</p></div>
              <div class="chain-stage"><b>03 · {{ l('别人的直接回应','Direct responses from others') }}</b><template v-if="step.responses.length"><p v-for="response in step.responses" :key="response.id"><small>Tick {{ response.tick }} · {{ response.speaker }} · {{ actionLabel(response.action) }}</small>{{ response.text }}</p></template><p v-else class="chain-empty">{{ l('截至当前时间步，尚无直接回复或引用。','No direct reply or quote has appeared by the current Tick.') }}</p></div>
              <div class="chain-stage chain-next"><b>04 · {{ l('后续行为','Subsequent behaviour') }}</b><p v-if="step.nextAction"><small>Tick {{ step.nextAction.tick }} · {{ actionLabel(step.nextAction.action) }}</small>{{ step.nextAction.text }}</p><p v-else class="chain-empty">{{ l('这是当前已发布轨迹中的最后一步。','This is the last step in the currently published trajectory.') }}</p></div>
            </li>
          </ol>
          <p v-else class="empty-state">{{ l('请选择一名已有连续公开轨迹的 Agent。','Select an Agent with a published multi-step trajectory.') }}</p>
        </section>
        ...
        <section v-else-if="tab==='profile'" class="section-stack profile-section">
          <header class="profile-heading">
            ...
            <div v-if="editable && projectId" class="profile-actions">
              ...
              <template v-else><button type="button" class="profile-quiet" :disabled="profileSaving" @click="cancelProfileEdit">{{ l('取消','Cancel') }}</button><button type="button" class="profile-primary" :disabled="profileSaving" @click="saveProfileDraft">{{ profileSaving ? l('正在冻结…','Freezing…') : l('保存人物设定','Save profile') }}</button></template>
...
.history-stream{display:grid;gap:.75rem;margin:0;padding:0;list-style:none}.history-stream li{display:grid;grid-template-columns:5rem 1fr;gap:.8rem}.history-stream time{padding-top:.6rem;color:#d4af58;font:750 .64rem var(--cp-font-mono)}.history-stream li>div{padding:.65rem .75rem;border:1px solid rgba(255,255,255,.09);background:rgba(255,255,255,.03)}.history-stream b{font-size:.7rem}.history-stream p{margin:.35rem 0;color:#d7cfd0;font-size:.72rem;line-height:1.55}.history-stream small{color:#988d90}.group-private-evidence{display:grid;gap:.55rem;padding-top:.8rem;border-top:1px solid rgba(255,255,255,.1)}.group-private-evidence h4{margin:0;font-size:.78rem}.group-private-evidence article{border-left-color:#d8af56}.group-private-evidence small{display:block;margin-top:.35rem;color:#a89b9e;font-size:.65rem}.chain-section>header p{margin:.4rem 0 0;color:#a99da0;font-size:.72rem}.chain-basis{border-left:3px solid #d4af58!important}.chain-basis b,.chain-stage>b{color:#e4bd68;font:800 .65rem var(--cp-font-mono);letter-spacing:.04em}.chain-basis p{margin:.45rem 0 0;color:#ddd4d5;font-size:.74rem;line-height:1.65}.behaviour-chain{display:grid;gap:1rem;margin:0;padding:0;list-style:none}.behaviour-chain>li{display:grid;grid-template-columns:4.5rem repeat(4,minmax(0,1fr));gap:.55rem;padding:.75rem;border:1px solid rgba(255,255,255,.1);background:#151113}.behaviour-chain>li>time{padding-top:.15rem;color:#efc76d;font:800 .66rem var(--cp-font-mono)}.chain-stage{min-width:0;padding:.7rem;border-top:2px solid #595054;background:rgba(255,255,255,.035)}.chain-stage.chain-action{border-top-color:#df1748;background:rgba(213,27,73,.08)}.chain-stage.chain-next{border-top-color:#6fa98f}.chain-stage p{margin:.5rem 0 0;color:#ddd4d5;font-size:.69rem;line-height:1.55}.chain-stage p+p{padding-top:.45rem;border-top:1px solid rgba(255,255,255,.07)}.chain-stage small{display:block;margin-bottom:.25rem;color:#94888b;font:650 .58rem var(--cp-font-mono)}.chain-stage .chain-empty{color:#84797c;font-style:italic}.dossier-enter-active,.dossier-leave-active{transition:opacity .22s}.dossier-enter-active .dossier-card,.dossier-leave-active .dossier-card{transition:transform .22s}.dossier-enter-from,.dossier-leave-to{opacity:0}.dossier-enter-from .dossier-card,.dossier-leave-to .dossier-card{transform:translateY(18px) scale(.985)}
@media(max-width:1100px){.behaviour-chain>li{grid-template-columns:4rem repeat(2,minmax(0,1fr))}.behaviour-chain>li>time{grid-row:1/3}}@media(max-width:700px){.behaviour-chain>li{grid-template-columns:1fr}.behaviour-chain>li>time{grid-row:auto}}
